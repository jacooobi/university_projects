...
ShaderProgram::ShaderProgram(char* vertexShaderFile, char* geometryShaderFile, char* fragmentShaderFile) {
	...
	printf("[%s] Loading vertex shader...\n", vertexShaderFile);
	vertexShader=loadShader(GL_VERTEX_SHADER, vertexShaderFile);

	//Wczytaj geometry shader
	if (geometryShaderFile!=NULL) {
		printf("[%s] Loading geometry shader...\n", geometryShaderFile);
	...
	printf("[%s] Loading fragment shader...\n", fragmentShaderFile);
...
void ShaderProgram::setUniform(char* variableName, GLsizei count, glm::mat4& value) {
    glUniformMatrix4fv(getUniformLocation(variableName), count, false, glm::value_ptr(value));
}

void ShaderProgram::setUniform(char *variableName, GLsizei count, glm::vec3 &value) {
    glUniform3fv(getUniformLocation(variableName), count, glm::value_ptr(value));
}

void ShaderProgram::setUniform(char *variableName, GLsizei count, glm::vec4 &value) {
    glUniform4fv(getUniformLocation(variableName), count, glm::value_ptr(value));
}

void ShaderProgram::setUniform(char *variableName, float value) {
    glUniform1f(getUniformLocation(variableName), value);
}

void ShaderProgram::setUniformInt(char *variableName, int value) {
